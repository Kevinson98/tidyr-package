...
table1
gather(table1, key="case",value = "Totals",cases,population)
...
pivot_longer(iris,-c(Species), names_to= "flower_att", values_to= "measurement")
...
stocks <- tibble(time = as.Date('2009-01-01') + 0:15,stock = rnorm(16, 0, 1)+15)
st<-stocks %>% separate(time,c("year","month","day"))

##unite()
st %>% unite("date",c("year","month","day"),sep = "-")
?unite
library(dplyr)
st %>% mutate(tada= stock/2)
## using chop() in tidyr
mouse %>% chop(c(size))
